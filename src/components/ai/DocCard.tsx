...
import { Badge, SaveIndicator } from '../ui';
      ...
      <div style={headerStyle}>
        ...
        {editing ? (
          <input
            ...
            onFocus={e => { e.currentTarget.style.borderColor = 'var(--accent, #B7FF00)'; e.currentTarget.style.boxShadow = '0 0 0 1px rgba(183,255,0,0.1)'; }}
            onBlur={e => { e.currentTarget.style.borderColor = 'var(--border-default, #2a2a2a)'; e.currentTarget.style.boxShadow = 'none'; }}
          />
        ) : (
          <span style={titleStyle}>{card.title}</span>
        )}
        {card.status && !editing && (
          <span style={{ fontSize: '0.625rem', color: 'var(--text-muted, #666)', flexShrink: 0 }}>
            {card.status}
          </span>
        )}
        {editing && (
        ...
        <div style={{ padding: 0 }}>
            ...
            <div key={i} style={sectionStyle}>
              {editing ? (
                ...
                <>
                  ...
                  <div style={richContentStyle}>{section.content}</div>
                </>
              )}
            </div>
          ))}
          {editing && (
            <div style={{ ...sectionStyle, display: 'flex', gap: 6 }}>
              <button
                style={{ ...actionBtnStyle, fontSize: '0.6875rem', border: '1px dashed var(--border-default, #2a2a2a)' }}
                onClick={() => setEditSections([...editSections, { title: '', content: '' }])}
              >
                + 添加章节
              </button>
            </div>
        ...
        {editing ? (
          <>
            <button
              ...
              onClick={handleSave}
              onMouseEnter={e => { e.currentTarget.style.background = 'rgba(183,255,0,0.18)'; e.currentTarget.style.borderColor = 'rgba(183,255,0,0.3)'; }}
              onMouseLeave={e => { e.currentTarget.style.background = 'rgba(183,255,0,0.1)'; e.currentTarget.style.borderColor = 'rgba(183,255,0,0.15)'; }}
            >
              保存
            </button>
          ...
          <>
            ...
            {onCollect && (
              <button style={actionBtnStyle} onClick={() => onCollect(card.id)}>
                收录为设定
              </button>
            )}
            {onExpand && (
              <button style={actionBtnStyle} onClick={() => onExpand(card.id)}>
                继续展开
              </button>
            )}
          </>
